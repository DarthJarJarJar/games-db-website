...
/* empty css                                                               */var index_svelte_svelte_type_style_lang = /* @__PURE__ */ (() => ".games.svelte-abfciu{display:grid;grid-template-columns:repeat(auto-fit, minmax(250px, 1fr));grid-column-gap:1rem;grid-row-gap:2rem;height:20vh;margin-top:2rem}h2.svelte-abfciu{margin-top:2rem}")();
const css = {
...
async function load({ fetch, params }) {
  const res = await fetch(`https://powerful-fjord-21607.herokuapp.com/https://api.igdb.com/v4/games/`, {
    ...
    body: `fields name, cover.image_id, follows; sort follows desc; where follows != null & first_release_date>${Math.floor(recentTimeStamp / 1e3)} & first_release_date<${Math.floor(currentDate.getTime() / 1e3)};`
  });
